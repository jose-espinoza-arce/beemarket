...
(function ($) {
    ...
    function Slider(container, settings) {
        ...
        if (this.settings.mode === "carousel") {
            var cloned_before = this.list.children().clone(true), cloned_after = this.list.children().clone(true);
    ...
    Slider.prototype.slideOffset = function (offset, force) {
        ...
        if (this.cssok) {
            ...
            if (this.settings.effect === "fade") {
                ...
                $.each(now_visible, function (n) {
                    $(this).stop().delay(self.settings.fadeDelay * n).animate({opacity: 0}, self.settings.fadeDuration, self.settings.fadeEasing, function () {
                        if (n < self.visible - 1) {
                            return
                        }
                        next_visible.css("opacity", 0);
                        self.list.delay(10).queue(function () {
                            ...
                            $.each(next_visible, function (n) {
                                $(this).stop().delay(self.settings.fadeDelay * n).animate({opacity: 1}, self.settings.fadeDuration, self.settings.fadeEasing, function () {
                                    if (n < self.visible - 1) {
                                        return
                                    }
                                    now_visible.add(next_visible).css("opacity", "");
                ...
                this.list.stop().delay(this.settings.slideDelay).animate({left: -pixel_offset}, this.settings.slideDuration, this.settings.slideEasing, unlock)
            }
        }
    };
    ...
    Slider.prototype.syncContainerHeight = function () {
        if (this.settings.syncHeight && !this.settings.itemKeepRatio) {
            ...
            var duration = this.settings.syncHeightDuration, easing = this.settings.syncHeightEasing;
            if (this.cssok) {
